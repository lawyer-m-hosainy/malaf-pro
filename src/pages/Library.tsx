...
export default function Library() {
  ...
  const categories = ['all', ...Array.from(new Set(libraryItems.map((item: any) => item.category)))];
    ...
    <>
      ...
      <div className="w-full">
        <div className="flex gap-2 justify-start overflow-x-auto overflow-y-hidden flex-nowrap pb-1 h-auto mb-4 border-b">
          ...
          {categories.filter(c => c !== 'all').map((category: any) => (
            <button 
              key={category} 
        ...
        {isLoading ? (
          <div className="text-center p-10 text-muted-foreground">جاري تحميل المكتبة...</div>
        ) : (
              ...
              <Card key={item.id} className="group hover:-translate-y-1 transition-all duration-200">
                <CardHeader className="p-4 pb-2">
                  ...
                  <CardDescription className="flex items-center gap-2 mt-2 text-xs">
                    <span>{(item.type || 'pdf').toUpperCase()}</span>
                    <span>•</span>
