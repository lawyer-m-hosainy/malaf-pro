        ...
        {stats.map((stat, idx) => (
          <Card key={idx}>
            <CardContent className="p-4 flex flex-col justify-center text-center">
              <span className="text-2xl font-bold font-mono">{isLoading ? '-' : stat.value}</span>
                      ...
                      <tr key={item.id} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                         ...
                         <td className="p-4">
                            <div className="flex items-center gap-2">
                               <span className="font-mono text-[13px]">{item.nextSession || 'يحدد لاحقاً'}</span>
                               {item.nextSession && item.nextSession !== '-' && item.nextSession !== 'يحدد لاحقاً' && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-2xl bg-background rounded-lg shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setIsAddModalOpen(false)} disabled={addCaseMutation.isPending}>
                <X className="h-4 w-4" />
              </Button>
            </div>
            
            <form onSubmit={handleSubmit(onSubmit)} className="p-4 overflow-y-auto">
              {addCaseMutation.isError && (
                <div className="mb-4 p-3 bg-destructive/10 text-destructive text-sm rounded-md border border-destructive/20">
                  حدث خطأ أثناء حفظ القضية. تأكد من صحة البيانات.
                </div>
              )}
              ...
              <div className="pt-4 border-t flex justify-end gap-3 mt-6">
                <Button type="button" variant="outline" onClick={() => setIsAddModalOpen(false)} disabled={addCaseMutation.isPending}>إلغاء</Button>
                <Button type="submit" className="gap-2" disabled={addCaseMutation.isPending}>
                  {addCaseMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : 'حفظ ملف القضية'}
                </Button>
              </div>
            </form>
          </div>
